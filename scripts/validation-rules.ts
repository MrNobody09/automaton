...
function hasBlockingCommand(workflow: WorkflowObject, command: string): boolean {
  return Object.values(workflow.jobs ?? {}).some((job: any) =>
    blockingCommandsForJob(job).includes(command),
  );
}
...
export function validatePackageScripts(packageJson: any): void {
  ...
  for (const misleadingName of ["test:security", "test:financial", "test:regression", "test:coverage"]) {
    if (misleadingName in scripts) {
      fail(`${misleadingName} must not exist unless it has a distinct, implemented contract.`);
    }
  }
...
export function validateCiWorkflow(workflow: WorkflowObject): void {
  ...
  if (full?.strategy?.["fail-fast"] !== false) {
    fail("CI isolated-full-suite must keep fail-fast: false.");
  }
  if (!Number.isFinite(full?.["timeout-minutes"]) || full["timeout-minutes"] > 15) {
    fail("CI isolated-full-suite must keep a hard timeout of 15 minutes or less.");
  }
  const fullCommands = blockingCommandsForJob(full);
  if (fullCommands.filter((command) => command.startsWith("tsx scripts/run-isolated-tests.ts")).length !== 1) {
    fail("CI isolated-full-suite must contain exactly one typed isolated-runner command.");
  }
  if (!fullCommands.includes("tsx scripts/run-isolated-tests.ts ${{ matrix.shard }} 4")) {
    fail("CI isolated-full-suite must invoke matrix.shard with shardTotal=4 exactly.");
  }
...
export function validateReleaseWorkflow(workflow: WorkflowObject): void {
  ...
  if (validation?.strategy?.["fail-fast"] !== false) {
    fail("Release validation must keep fail-fast: false.");
  }
...
export function validateShardPartition(files: readonly string[], shardTotal = 4): void {
  const shards = Array.from({ length: shardTotal }, (_, index) =>
    selectTestShard(files, index + 1, shardTotal),
  );
  if (shards.some((shard) => shard.length === 0)) {
    fail(`${shardTotal}-shard partition must not contain an empty shard.`);
  }
  ...
  if (flattened.some((file) => !source.has(file))) {
    fail("Shard partition contains a file outside canonical discovery.");
  }
}
