...
const contractControls = [
  ...
  {
    label: "validation contract rejects direct Vitest test scripts",
    file: "package.json",
  ...
  {
    label: "validation contract rejects newly added direct Vitest test scripts",
    file: "package.json",
    replacements: [['"test:ci": "node scripts/run-isolated-tests.mjs",', '"test:ci": "node scripts/run-isolated-tests.mjs",\n    "test:coverage": "vitest run --coverage",']],
    requiredText: "test:coverage must route through the process-isolated test runner",
  },
  {
  ...
  {
    label: "validation contract rejects missing loaded network guard",
    file: "vitest.config.ts",
    replacements: [['setupFiles: ["./test/setup/network-guard.ts"],', 'setupFiles: [],']],
    requiredText: "must load test/setup/network-guard.ts",
  },
  {
    label: "validation contract rejects allowOnly being enabled",
    file: "vitest.config.ts",
    replacements: [["allowOnly: false,", "allowOnly: true,"]],
    requiredText: "must set allowOnly=false",
  },
  {
    label: "validation contract ignores comments and rejects commented-out release sensitivity",
    file: ".github/workflows/release.yml",
    replacements: [["        run: node scripts/verify-test-sensitivity.mjs", "        # run: node scripts/verify-test-sensitivity.mjs\n        run: echo sensitivity-disabled"]],
    requiredText: "Release workflow is missing blocking command: node scripts/verify-test-sensitivity.mjs",
  },
  {
    label: "validation contract rejects nonblocking required release gates",
    file: ".github/workflows/release.yml",
    replacements: [["      - run: pnpm run typecheck:tests", "      - run: pnpm run typecheck:tests\n        continue-on-error: true"]],
    requiredText: "Release workflow is missing blocking command: pnpm run typecheck:tests",
  },
  {
    label: "validation contract rejects fail-open dependency audit",
    file: ".github/workflows/release.yml",
    replacements: [["      - run: pnpm audit --audit-level=high", "      - run: pnpm audit --audit-level=high || true"]],
    requiredText: "Release workflow is missing blocking command: pnpm audit --audit-level=high",
  },
  {
    ...
    requiredText: "action must be pinned to an exact commit SHA",
  },
  {
    ...
    requiredText: "must set persist-credentials: false",
  },
  {
    label: "validation contract rejects broadened workflow permissions",
    file: ".github/workflows/release.yml",
    replacements: [["permissions:\n  contents: read", "permissions:\n  contents: write"]],
    requiredText: "workflow permissions must be exactly contents: read",
  },
  {
    label: "validation contract rejects workflow-level test network opt-out",
    file: ".github/workflows/release.yml",
    replacements: [["jobs:\n  release:", "env:\n  AUTOMATON_TEST_ALLOW_NETWORK: 1\n\njobs:\n  release:"]],
    requiredText: "must not enable AUTOMATON_TEST_ALLOW_NETWORK",
  },
];
