  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (method === "GET" && url.pathname === "/v1/audit") {
        ...
        json(res, 200, {
          controlAudit: getOwnerControlAudit(db.raw, Number.isFinite(limit) ? limit : 50),
          policyDecisions: recentPolicyDecisions(db, Number.isFinite(limit) ? limit : 50),
          modifications: db.getRecentModifications(Math.min(Math.max(Number.isFinite(limit) ? limit : 50, 1), 200)),
        });
        return;
      }
...
<script>
function saveToken(){sessionStorage.setItem('ownerToken',document.getElementById('token').value);refresh()}
